...
TEST_F(TensorTest,Entry) {
  ...
  EXPECT_DOUBLE_EQ(0, T1->entry( vector<int>({1,2}),
				 vector<int>({1,0,3}) ).imag() );
  EXPECT_DOUBLE_EQ(1, T1->entry( {0,2}, {0,0,2} ).real() );
  EXPECT_DOUBLE_EQ(0, T1->entry( {0,2}, {0,0,2} ).imag() );
  EXPECT_DOUBLE_EQ(1, T1->entry( vector<int>({0,2}),
				 vector<int>({0,0,2}) ).real() );
  EXPECT_DOUBLE_EQ(0, T1->entry( vector<int>({0,2}),
				 vector<int>({0,0,2}) ).imag() );

  // test setting and retrieving values, exercizing both methods
  T1->set_entry( vector<int>({3,5}), vector<int>({2,0,4}),
		 complex<double>(2.4,3.6) );
  EXPECT_DOUBLE_EQ(2.4,T1->entry( vector<int>({3,5}),
				  vector<int>({2,0,4})
				  ).real() );
